  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{t("settings.title")} — {project?.name}</DialogTitle>
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">{t("settings.spaModeDesc")}</p>
              </div>
              <Switch
                id="spa-mode"
                checked={settings.spaMode}
                onCheckedChange={(checked) => setSettings((s) => ({ ...s, spaMode: checked }))}
              />
            </div>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <div className="text-left min-w-0">
                    <p className="text-xs font-medium">{t("settings.routingHash")}</p>
                    <p className={`text-[10px] mt-0.5 whitespace-normal ${settings.routingType === "hash" ? "text-primary-foreground/70" : "text-muted-foreground"}`}>
                ...
                >
                  ...
                  <div className="text-left min-w-0">
                    <p className="text-xs font-medium">{t("settings.routingPath")}</p>
                    <p className={`text-[10px] mt-0.5 whitespace-normal ${settings.routingType === "path" ? "text-primary-foreground/70" : "text-muted-foreground"}`}>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between gap-3 p-3 rounded-lg border border-destructive/20">
                <div className="space-y-0.5 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{t("settings.deleteProjectDesc")}</p>
                </div>
                {!confirmDelete ? (
