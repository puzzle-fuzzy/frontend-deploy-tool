...
状态：已于 2026-07-30 完成后端首版。当前详细报告按版本持久化，运行摘要进入
长期历史；项目 owner 可在默认仅提示与显式阻断之间切换。管理面板中的可视化
报告入口留待下一轮前端迭代，不影响 API、发布门禁或恢复能力。
...
完成门槛：审计器失败不会损坏上传；用户能看到可解释的检查结果；阻止
发布必须由项目策略显式开启。

验证证据：全仓 `bun run verify` 通过（server 266、client 40、desktop 23）；
生产双域真实进程完成“上传 → 缺失报告阻断 → 审计 → 发布第二版 → 手动回退”，
指标记录两次 warning 报告；SIGINT/SIGTERM 均完成 drain/checkpoint，使用同一
SQLite 重启后报告 ID 与线上版本指针保持一致。
